    ...
    * Hand crafted features (some new and some from previously published classifiers)
    ...
    def __init__(self,
        ...
        if self.features == 'all' or 'all' in self.features:
            self.features = list(self.train_feature_indices.keys())
        ...
        if var_name:
            for column in f[var_name]:
                ...
                for row_number in range(len(column)):
                    row_data.append(''.join(map(chr, f[column[row_number]][:].flatten())))
                var.append(row_data)
        ...
        # if not, create it
            ...
            with h5py.File(join(self.datapath, 'features', 'features_PSD_med_var_kurt.mat'), 'r') as f:
                ...
                for element in f['features_out'][0]:
                    ...
                    if data.ndim == 1 or data.dtype != np.float64:
                        continue
                    nyquist = old_div((data.shape[1] - 2), 3)
                    nfreq = 100
                    # if more than nfreqs, remove extra
                    if nyquist > nfreq:
                        data = data[:, np.concatenate((list(range(2 + nfreq)),
                                                      list(range(2 + nyquist, 2 + nyquist + nfreq)),
                                                      list(range(2 + 2*nyquist, 2 + 2*nyquist + nfreq))))]
                    # if less than nfreqs, repeat last frequency value
                    elif nyquist < nfreq:
                        data = data[:, np.concatenate((list(range(2 + nyquist)),
                                                       np.repeat(1 + nyquist, nfreq - nyquist),
                                                       list(range(2 + nyquist, 2 + 2*nyquist)),
                                                       np.repeat(1 + 2*nyquist, nfreq - nyquist),
                                                       list(range(2 + 2*nyquist, 2 + 3*nyquist)),
            ...
            if self.label_type == 'database':
                ...
                with open(join(self.datapath, 'labels', file_name), 'rb') as f:
                    ...
                    data = pkl.load(f)
                    if 'transform' in list(data.keys()):
                    ...
                    if isinstance(labels, np.ndarray):
                        labels = [labels]
                    if 'labels_cov' in list(data.keys()):
            ...
            label2component = dict(list(zip(*temp)))
            del temp
            # match feature-sets
            ...
            if self.label_type == 'database':
                ...
                if 'labeled_label_covariances' in list(dataset.keys()):
    ...
    def load_semi_supervised(self):
        ...
        if self.label_type == 'database':
            ...
            if 'labeled_label_covariances' in list(icl.keys()):
        ...
        ind = rng.permutation(np.setdiff1d(list(range(icl['unlabeled_features']['ids'].shape[0])), test_ind))
        x_u = OrderedDict([(key, val[ind]) for key, val in icl['unlabeled_features'].items()])
        y_u = None
        # labeled training set
        ind = rng.permutation(np.setdiff1d(list(range(icl['labeled_features']['ids'].shape[0])), valid_ind))
    ...
    def _unscale(vec, min, max):
        return old_div((vec + 1) * (max-min), 2) + min
    ...
    def normalize_dipole_features(data):
        ...
        max_dist = 1.5
        data[:, ind_dipole_pos] = old_div(np.clip(data[:, ind_dipole_pos], -max_dist, max_dist), max_dist)
    ...
    def _plot_grid(data, function):
        ...
        b = np.ceil(old_div(1. * nax, a)).astype(np.int)
